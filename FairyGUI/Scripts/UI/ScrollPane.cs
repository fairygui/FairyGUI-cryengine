...
namespace FairyGUI
{
	...
	public class ScrollPane : EventDispatcher
	{
		...
		public void SetPercX(float value, bool ani)
		{
			...
			SetPosX(_overlapSize.x * MathHelpers.Clamp01(value), ani);
		}
		...
		{
			_owner.EnsureBoundsCorrect();
			SetPosY(_overlapSize.y * MathHelpers.Clamp01(value), ani);
		}
		...
		public void SetPosX(float value, bool ani)
		{
			...
			value = MathHelpers.Clamp(value, 0, _overlapSize.x);
			if (value != _xPos)
			{
				_xPos = value;
				PosChanged(ani);
		...
		public void SetPosY(float value, bool ani)
		{
			...
			value = MathHelpers.Clamp(value, 0, _overlapSize.y);
			if (value != _yPos)
			{
				_yPos = value;
				PosChanged(ani);
		...
		public float scrollingPosX
		{
			get
			{
				return MathHelpers.Clamp(-_container.x, 0, _overlapSize.x);
			}
		}
		...
		public float scrollingPosY
		{
			get
			{
				return MathHelpers.Clamp(-_container.y, 0, _overlapSize.y);
			}
		}
		...
		public void LockHeader(int size)
		{
			...
			{
				_tweenStart = (Vector2)_container.position;
		...
		internal void AdjustMaskContainer()
		{
			...
			_maskContainer.SetPosition(mx, my, 0);
		}
		...
		{
			if (MathHelpers.Approximately(_contentSize.x, aWidth) && MathHelpers.Approximately(_contentSize.y, aHeight))
				return;
		...
		void HandleSizeChanged()
		{
			...
			//边界检查
			_xPos = MathHelpers.Clamp(_xPos, 0, _overlapSize.x);
			_yPos = MathHelpers.Clamp(_yPos, 0, _overlapSize.y);
			float max = _overlapSize[_refreshBarAxis];
			if (max == 0)
				max = Math.Max(_contentSize[_refreshBarAxis] + _footerLockedSize - _viewSize[_refreshBarAxis], 0);
			else
			...
			if (_refreshBarAxis == 0)
				_container.SetPosition(MathHelpers.Clamp(_container.x, -max, _headerLockedSize), MathHelpers.Clamp(_container.y, -_overlapSize.y, 0), 0);
			else
				_container.SetPosition(MathHelpers.Clamp(_container.x, -_overlapSize.x, 0), MathHelpers.Clamp(_container.y, -max, _headerLockedSize), 0);
		...
		private void PosChanged(bool ani)
		{
			...
			Stage.beforeVisit -= _refreshDelegate;
			Stage.beforeVisit += _refreshDelegate;
		}
		...
		private void SyncScrollBar(bool end = false)
		{
			if (_vtScrollBar != null)
			{
				_vtScrollBar.scrollPerc = _overlapSize.y == 0 ? 0 : MathHelpers.Clamp(-_container.y, 0, _overlapSize.y) / _overlapSize.y;
				if (_scrollBarDisplayAuto)
					ShowScrollBar(!end);
			}
			if (_hzScrollBar != null)
			{
				_hzScrollBar.scrollPerc = _overlapSize.x == 0 ? 0 : MathHelpers.Clamp(-_container.x, 0, _overlapSize.x) / _overlapSize.x;
				if (_scrollBarDisplayAuto)
					ShowScrollBar(!end);
			}
		}
		...
		private void __touchMove(EventContext context)
		{
			...
			//更新速度
			float deltaTime = FrameTime.Delta;
			float elapsed = (Engine.Timer.GetCurrTime() - _lastMoveTime) * 60 - 1;
			if (elapsed > 1) //速度衰减
			...
			//同步更新pos值
			if (_overlapSize.x > 0)
				_xPos = MathHelpers.Clamp(-_container.x, 0, _overlapSize.x);
			if (_overlapSize.y > 0)
				_yPos = MathHelpers.Clamp(-_container.y, 0, _overlapSize.y);
		...
		void LoopCheckingNewPos(ref float value, int axis)
		{
			...
			if (value < 0.001f)
			{
				...
				if (value > pos)
				{
					...
					pos = MathHelpers.Clamp(pos + v, 0, _overlapSize[axis]);
					changed = true;
				}
			}
		...
		float AlignByPage(float pos, int axis, bool inertialScrolling)
		{
			...
			if (pos > 0)
				...
				page = (int)Math.Ceiling(_contentSize[axis] / _pageSize[axis]) - 1;
			else
			{
				page = (int)Math.Floor(-pos / _pageSize[axis]);
				float change = inertialScrolling ? (pos - _containerPos[axis]) : (pos - _container.xy[axis]);
				float testPageSize = Math.Min(_pageSize[axis], _contentSize[axis] - (page + 1) * _pageSize[axis]);
			...
			if (inertialScrolling)
			{
				...
				if (oldPos > 0)
					...
					oldPage = (int)Math.Ceiling(_contentSize[axis] / _pageSize[axis]) - 1;
				else
					oldPage = (int)Math.Floor(-oldPos / _pageSize[axis]);
				int startPage = (int)Math.Floor(-_containerPos[axis] / _pageSize[axis]);
		...
		void FixDuration(int axis, float oldChange)
		{
			...
			float newDuration = Math.Abs(_tweenChange[axis] / oldChange) * _tweenDuration[axis];
			...
			if (_footer != null)
			{
				...
				if (pos < -max || max == 0 && _footerLockedSize > 0)
				{
					...
					if (max > 0)
						...
						vec[_refreshBarAxis] = Math.Max(Math.Min(pos + _viewSize[_refreshBarAxis], _viewSize[_refreshBarAxis] - _footerLockedSize), _viewSize[_refreshBarAxis] - _contentSize[_refreshBarAxis]);
					_footer.xy = vec;
		...
		void TweenUpdate(object param)
		{
			...
			{
				if (_overlapSize.x > 0)
					_xPos = MathHelpers.Clamp(-nx, 0, _overlapSize.x);
				if (_overlapSize.y > 0)
					_yPos = MathHelpers.Clamp(-ny, 0, _overlapSize.y);
		...
		float RunTween(int axis)
		{
			...
			{
				_tweenTime[axis] += FrameTime.Delta;
