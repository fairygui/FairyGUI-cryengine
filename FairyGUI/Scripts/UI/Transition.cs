...
namespace FairyGUI
{
	...
	public class TransitionItem_Sound : TransitionItem
	{
		...
		{
			if (audioClip == null)
				audioClip = UIPackage.GetItemAssetByURL(sound) as AudioFile;
	...
	public class TransitionItem_Shake : TransitionItem
	{
		...
		{
			float r = (float)Math.Ceiling(shakeAmplitude * elapsed / shakePeriod);
			...
			target._gearLocked = true;
			target.SetPosition(target.x - offsetX + vr.x, target.y - offsetY + vr.y);
			target._gearLocked = false;

			offsetX = vr.x;
			offsetY = vr.y;
			elapsed -= FrameTime.Delta;
			if (elapsed <= 0)
			{
				target._gearLocked = true;
				target.SetPosition(target.x - offsetX, target.y - offsetY);
