	...
	public class Transition
	{
		...
		void StopItem(TransitionItem item, bool setToComplete)
		{
			...
			else if (item.type == TransitionActionType.Shake)
			{
				if (item.onShake != null)
				{
					...
					item.target.SetPosition(item.target.x - item.startValue.f1, item.target.y - item.startValue.f2);
		...
		void ApplyValue(TransitionItem item, TransitionValue value)
		{
			...
			switch (item.type)
			{
				...
				case TransitionActionType.Sound:
					AudioFile sound = UIPackage.GetItemAssetByURL(value.s) as AudioFile;
					if (sound != null)
		...
		{
			float r = (float)Math.Ceiling(item.value.f1 * item.startValue.f3 / item.value.f2);
			Vector2 vr = new Vector2(Random.Range(0, r), Random.Range(0, r));
			vr.x = vr.x > 0 ? (float)Math.Ceiling(vr.x) : (float)Math.Floor(vr.x);
			vr.y = vr.y > 0 ? (float)Math.Ceiling(vr.y) : (float)Math.Floor(vr.y);

			item.target._gearLocked = true;
			item.target.SetPosition(item.target.x - item.startValue.f1 + vr.x, item.target.y - item.startValue.f2 + vr.y);
			item.target._gearLocked = false;

			item.startValue.f1 = vr.x;
			item.startValue.f2 = vr.y;
			item.startValue.f3 -= FrameTime.Delta;
