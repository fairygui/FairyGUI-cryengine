...
#else
using DOVector2 = CryEngine.Vector2;
using DOVector3 = CryEngine.Vector3;
using DOVector4 = CryEngine.Vector4;
using DOQuaternion = CryEngine.Quaternion;
using DOColor = CryEngine.Color;
#endif
...
namespace DG.Tweening
{
    ...
    public static class TweenSettingsExtensions
    {
        ...
        public static T SetLoops<T>(this T t, int loops) where T : Tween
        {
            ...
            t.loops = loops;
//            if (t.tweenType == TweenType.Tweener) t.fullDuration = loops > -1 ? t.duration * loops : MathHelpers.Infinity; // Mysteriously Unity doesn't like this form
            if (t.tweenType == TweenType.Tweener) {
                if (loops > -1) t.fullDuration = t.duration * loops;
                else t.fullDuration = float.PositiveInfinity;
            }
            return t;
        }
        /// <summary>Sets the looping options for the tween. 
        ...
        public static T SetLoops<T>(this T t, int loops, LoopType loopType) where T : Tween
        {
            ...
            t.loops = loops;
            t.loopType = loopType;
//            if (t.tweenType == TweenType.Tweener) t.fullDuration = loops > -1 ? t.duration * loops : MathHelpers.Infinity;
            if (t.tweenType == TweenType.Tweener) {
                if (loops > -1) t.fullDuration = t.duration * loops;
                else t.fullDuration = float.PositiveInfinity;
            }
