...
//            int toCompletedLoops = (int)(to / t.duration); // With very small floats creates floating points imprecisions
            int toCompletedLoops = (int)Math.Floor(to / t.duration); // Still generates imprecision with some values (like 0.4)
...
//            int increaseSequencesBy = _DefaultMaxSequences;
            int increaseTweenersBy = MathHelpers.Max((int)(maxTweeners * 1.5f), _DefaultMaxTweeners);
            int increaseSequencesBy = MathHelpers.Max((int)(maxSequences * 1.5f), _DefaultMaxSequences);
            switch (increaseMode) {
...
//            maxActive = MathHelpers.Max(maxTweeners, maxSequences);
