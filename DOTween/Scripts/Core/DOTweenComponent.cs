...
{
	public class DOTweenProgram : ICryEnginePlugin
	{
		public override void Initialize()
		{
			DOTweenComponent.Create();
			GameFramework.RegisterForUpdate(DOTween.instance);
		}

		public override void Shutdown()
		{
			if (DOTween.instance != null)
				GameFramework.UnregisterFromUpdate(DOTween.instance);
	...
	/// </summary>
	//[AddComponentMenu("")]
	public class DOTweenComponent : IDOTweenInit, IGameUpdateReceiver
	{
		...
		{
			_unscaledDeltaTime = FrameTime.Delta;//Time.realtimeSinceStartup - _unscaledTime;
			...
			{
				TweenManager.Update(UpdateType.Normal, _unscaledDeltaTime/*(DOTween.useSmoothDeltaTime ? Time.smoothDeltaTime : Time.deltaTime)*/ * DOTween.timeScale, _unscaledDeltaTime * DOTween.timeScale);
			}
			//_unscaledTime = Time.realtimeSinceStartup;

			if (DOTween.isCryEditor)
			{
				inspectorUpdater++;
			...
			{
				TweenManager.Update(UpdateType.Late, FrameTime.Delta/* (DOTween.useSmoothDeltaTime ? Time.smoothDeltaTime : Time.deltaTime)*/ * DOTween.timeScale, _unscaledDeltaTime * DOTween.timeScale);
			}
		}
		/*
        void FixedUpdate()
		...
		/*void OnDrawGizmos()
        {
            if (!DOTween.drawGizmos || !DOTween.isCryEditor) return;
		...
		internal static void Create()
		{
			...
			DOTween.instance = new DOTweenComponent();
		}

		internal static void DestroyInstance()
		{
			if (DOTween.instance != null) DOTween.instance.Destroy();
			DOTween.instance = null;
		}
	}
}